"""Subspace-Net main script 
    ...
    This scripts calls the following functions:
        ...
        * training: For training Droot_music model
...
def run_exp(num_sources, eta):
    if EVAL_ONLY:
        scenario_dict = {
            "coherent": range(-8,11),
            "non-coherent": range(-8,11),
        }
    else:
        scenario_dict = {
            "coherent": [range(-8,4)] # [range(4,11)], # ,
            #"non-coherent": [range(-8,4)],# [range(4,11)], #
        }
    ...
    system_model_params = {
        "M": num_sources if (num_sources and SANITY_CHECK_NUM_SOURCES) else None,  # number of sources
    ...
    training_params = {
        "samples_size": 1024 if EVAL_ONLY else (1024 * 8 if PIPE_CLEAN else 1024 * 100), 
        "train_test_ratio": .5,
...
if __name__ == "__main__":
    num_virtual_sensors = SensorsArray(f"MRA-{MRA_NUM_SENSORS}" ).last_sensor_loc
    if EVAL_ONLY:
        num_sources_opts = range(num_virtual_sensors)
        eta_opts = [0, 0.05]
    else:
        num_sources_opts = [0] # [0]
        eta_opts = [0.05]# [0, 0.05]
    for eta in eta_opts:
        for num_sources in num_sources_opts:
            run_exp(num_sources,eta)